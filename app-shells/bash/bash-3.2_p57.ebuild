...
PATCHES=(
	...
	# loadhigh: Fix CFLAGS_FOR_BUILD to use CFLAGS for C23 compatibility
	"${FILESDIR}"/${PN}-3.2-cflags-for-build.patch
)
...
src_configure() {
	...
	# loadhigh: Use /etc/bash/bashrc-3.2 for bash 3.2 compatibility
	append-cppflags \
		-DDEFAULT_PATH_VALUE=\'\"/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin\"\' \
		-DSTANDARD_UTILS_PATH=\'\"/bin:/usr/bin:/sbin:/usr/sbin\"\' \
		-DSYS_BASHRC=\'\"/etc/bash/bashrc-3.2\"\' \
...
src_install() {
	...
	# loadhigh: Install bash 3.2 compatible system bashrc
	insinto /etc/bash
	newins "${FILESDIR}"/bashrc-3.2 bashrc-3.2
}
