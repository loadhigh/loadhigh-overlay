...
bootstrap_uri() {
	local baseuri="https://dev.gentoo.org/~arthurzam/distfiles/dev-java/openjdk/openjdk-bootstrap"
	local suff="tar.xz"
...
pkg_setup() {
	...
	JAVA_PKG_WANT_BUILD_VM="sapmachine-${SLOT} sapmachine-jdk-bin-${SLOT}"
...
src_configure() {
	if has_version dev-java/sapmachine:${SLOT}; then
		...
		JDK_HOME=$(best_version -b dev-java/sapmachine-jdk-bin:${SLOT})
